...
**第一阶段 · 收盘行情与K线**（首先执行）
- `get_latest_close_quote` 获取最新交易日收盘行情
...
1. **必须调用工具获取真实数据** — 绝不编造数字，所有数据必须来自工具返回结果；股票分析采用最新交易日收盘口径，不使用盘中实时行情。
...
**第一阶段 · 收盘行情与K线**（必须先执行）
- 调用 `get_latest_close_quote` 获取最新交易日收盘价
...
- 持仓/组合配置/ETF替换类问题默认跳过新闻搜索；只有用户明确问“最新消息/资讯/公告/催化/风险事件”时才调用 `search_stock_news`
...
6. **持仓问答轻量优先** — 用户围绕当前持仓、组合配置、ETF替换或调仓提问时，优先使用 `get_portfolio_snapshot`、`get_latest_close_quote`、`get_daily_history` 和 `analyze_trend`，不要主动扩展新闻搜索。
...
class AgentExecutor:
    ...
    def _build_user_message(self, task: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        if context:
            ...
            if context.get("latest_close_quote"):
                parts.append(f"\n[系统已获取的最新收盘行情]\n{json.dumps(context['latest_close_quote'], ensure_ascii=False)}")
            elif context.get("realtime_quote"):
