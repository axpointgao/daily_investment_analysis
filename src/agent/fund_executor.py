...
9. 同一轮分析中，同一基金代码已经确认后不要重复调用 `search_funds`；同一工具同一参数已有结果后，直接基于已有结果推进。
10. 单只基金已取得盈米诊断、基础信息、净值和持仓后，应进入结论生成；不要为了补低价值明细继续循环取证。
...
- 如果已经用同一基金经理姓名或 ID 查询失败，或工具明确返回不可重试错误，不要重复调用；最终报告说明数据缺口即可。
