...
- 银行理财按金额口径记录，不要求份额和单位净值。首次买入记录银行名称、产品名称、买入支付金额和购买日期，登记编码、投资性质、风险等级、收益方式及期限日期可选；追加和赎回必须从当前持仓理财产品下拉选择，避免手输产品名造成重复持仓。银行流水的顶部日期是流水日期；产品期限日期中的起息日和到期日用于定期存款，银行理财可按产品实际情况选填。银行理财当前不接外部数据源，后续可在 Web 端“价值更新”入口选择持仓产品并录入当前总价值；未更新价值前按买入/追加/赎回流水金额估算。
