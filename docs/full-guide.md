...
- 投顾组合当前不自动读取陆基金/陆金所账户数据。最新估值需要在 Web 端“净值更新”入口选择投顾产品后手工维护；估值按“持有份额 × 最新单位净值”计算，未维护净值前会使用最近一次确认净值。配置 `YINGMI_API_KEY` 后，持仓“深度诊断”和诊基 Agent 会优先使用盈米 StarGate 的投顾策略、组合诊断和基金专业分析能力，手工账本仍作为资产金额与持仓事实来源。
