...
| 变量名 | 说明 | 默认值 | 必填 |
|--------|------|--------|:----:|
| `YINGMI_API_KEY` | 盈米 StarGate API Key。配置后作为首页基金专业诊断、诊基专业分析、持仓组合深度诊断、投顾策略与财富规划的优先专业引擎 | - | 否 |
| `YINGMI_ENABLED` | 是否启用盈米投顾能力；关闭后基金投顾流程只使用本地与天天基金数据 | `true` | 否 |
| `YINGMI_FUND_ANALYSIS_DEPTH` | 基金投顾分析深度：`fast` 更省额度；`professional` 默认推荐；`deep` 在组合/持仓场景增加风险、相关性和回测类检查 | `professional` | 否 |
| `YINGMI_FUND_DATA_STRATEGY` | 基金数据策略：`balanced` 盈米主判断并用基础数据补事实；`yingmi_only` 减少非盈米补充；`basic_only` 跳过盈米专业诊断 | `balanced` | 否 |
| `YINGMI_MCP_DAILY_LIMIT` | 盈米 MCP/OpenAPI 日额度，仅用于展示和后续预算控制 | `1000` | 否 |
| `YINGMI_SKILL_DAILY_LIMIT` | 盈米 remote-skill 日额度，仅用于展示；当前项目尚未提供长报告生成入口 | `100` | 否 |
| `TTFUND_APIKEY` | 天天基金 Skills API Key，用于基金搜索、基础资料、经理、持仓、交易规则和债市等补充数据 | - | 否 |
| `IWENCAI_API_KEY` | 问财 Skills/OpenAPI Key，当前仅用于银行理财产品查询和单位净值获取 | - | 否 |
| `IWENCAI_BASE_URL` | 问财 OpenAPI 地址 | `https://openapi.iwencai.com` | 否 |
| `TIANTIAN_FUND_API_BASE_URL` | 自建/三方 TiantianFundApi 地址，用于净值、历史表现和持仓估值等高频基础数据 | - | 否 |
...
| `PORTFOLIO_ANALYSIS_PROMPT_INSURANCE_BASIC` | 内置模板 | 保险账户资产专项体检写作要求；只分析现金价值、缴费压力、领取/返还和流动性，不做保障责任、理赔或购买建议 |

---

## 本地 WebUI 管理界面

WebUI 与 FastAPI API 服务共用同一服务进程，启动后可在浏览器中完成配置管理、手动分析、任务进度查看、历史报告、回测、持仓管理和智能导入等操作。前端界面使用 shadcn/ui 默认浅色组件样式；认证、云服务器访问和 API 调用细节见下方说明。
...
| `/api/v1/portfolio/tags` | GET/POST | 查询或新增持仓产品标签 |
| `/api/v1/portfolio/tags/{tag_id}` | PATCH/DELETE | 修改或删除持仓产品标签 |
| `/api/v1/portfolio/product-tags` | PUT | 为全局产品绑定或移除单个标签 |
...
- 账户现金采用分层口径：股票、场外基金和数字货币账户默认使用 `managed` 模式，买入/卖出会联动现金余额；银行、投顾和保险账户默认使用 `asset_only` 模式，买入、追加、缴费、赎回和领取只作为产品现金流与估值事实，不自动生成账户现金余额，避免用户未先录入入金时总资产被负现金抵消。需要严格现金账本时，可在 API 中把账户 `cash_tracking_mode` 设为 `managed`。
- 持仓明细会展示“年化”列，按该产品在当前账户内的买入、追加、赎回、分红、领取/返还和当前市值组成现金流后计算 XIRR；价格、汇率或现金流不足时显示为空值。价值更新和净值更新只影响期末估值，不单独作为现金流。
- 持仓标签是全局产品属性，可在设置页“持仓标签”中维护。持仓明细的资产辅助说明行会显示 `+标签` 或已选标签名，点击后可为该产品单选标签；同一个产品跨账户共用标签。资产分布默认按“资产属性”展示非现金资产金额和占比，未设置标签的资产归入“未定义”，也可切换回按“资产类型”展示。
- 持仓资产分析会按现有标签名识别四笔钱规划：`灵活备用`、`长期压舱`、`稳健增强`、`进取高增`。该能力只读取已有标签和持仓归属，不创建、不重命名、不自动改写标签；报告会基于 14% / 57% / 17% / 11% 的比例目标做配置合理性诊断，重点提示哪类偏多偏少、哪些资产属性与当前归属不匹配，以及可观察、补标签、调整归属、用新增资金再平衡、减持或替换等非交易指令式建议。
- 持仓快照会在 `positions[]` 中返回 `valuation_model`、`cost_display_value`、`price_display_value` 等展示辅助字段，用于区分单位净值型、金额价值型和保单现金价值型资产；兼容字段 `quantity`、`avg_cost`、`last_price` 仍保留，但不同资产类型下不要直接按同一金融含义解释。
- 银行账户支持活期/现金、定期存款和银行理财三类录入。定期存款按每笔存入流水形成独立产品，记录本金、起息日、到期日和年化利率，估值暂按本金计算；取出时从当前未结清定期产品下拉选择，避免同名产品误合并。
- 银行理财支持问财辅助录入：配置 `IWENCAI_API_KEY` 后，新买入可输入完整产品名查询银行理财产品，选择产品后按确认日单位净值自动计算份额；追加同样可记录确认份额，刷新持仓时有份额的理财产品可用问财最新单位净值估算当前价值。问财未配置、查询失败或产品无净值时，仍可按金额保存流水，并在 Web 端“价值更新”入口选择持仓产品手动录入当前总价值。
- 投顾是独立账户类型，不归入银行账户。Web 端按产品记录平台、产品名称、可选产品代码/外部 ID，并在产品类型中选择“投顾组合”或“定投计划”。配置 `YINGMI_API_KEY` 后，新产品买入可按产品名称查询盈米 StarGate 候选产品；确认候选后，投顾组合会按用户填写的流水日期查询历史净值并自动计算确认份额，定投计划只记录产品信息用于资产分析，不进入净值自动估值。查询失败、未配置或流水日期附近没有历史净值时不阻断提交，仍可手工保存金额型投顾流水。
- 投顾组合追加和赎回必须从既有投顾组合中选择；定投计划跟投和赎回/止盈必须从既有定投计划中选择，避免产品名手输错误生成重复持仓。已匹配盈米历史净值的投顾组合可在刷新持仓时使用盈米最新净值估算当前价值；未匹配净值的投顾组合和定投计划按金额口径顺推，用户可在 Web 端“价值更新”入口选择投顾产品并录入当前总价值进行手动纠偏。投顾产品当前不自动读取且慢、支付宝、陆基金/陆金所等账户数据。底层基金穿透本轮不提供入口；如用户未来选择逐只录入底层基金，应避免同时把投顾产品总价值重复计入。配置 `YINGMI_API_KEY` 后，持仓“深度诊断”和诊基 Agent 会优先使用盈米 StarGate 的投顾策略、组合诊断和基金专业分析能力，手工账本仍作为资产金额与持仓事实来源。
