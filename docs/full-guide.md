...
- 银行理财记录产品名称、登记编码、金额、买入确认份额、买入确认净值和派息/滚存，投资性质与风险等级可选；赎回时从当前持仓理财产品下拉选择并填写赎回份额和金额。银行流水的顶部日期是流水日期；产品期限日期中的起息日和到期日用于定期存款，银行理财可按产品实际情况选填。银行理财当前不接外部数据源，买入确认净值会同步保存为当日手工净值，后续最新净值可在 Web 端“净值更新”入口选择持仓产品后维护；估值按“持有份额 × 最新净值”计算。
- 投顾是独立账户类型，不归入银行账户。Web 端按产品记录平台、产品名称、可选产品代码/外部 ID，并在产品类型中选择“投顾组合”或“定投计划”。投顾组合区分“买入新组合”和“追加买入”：新组合手动填写产品信息，追加和赎回必须从既有投顾组合中选择；定投计划的首次买入手动填写产品信息，定投投入、手动跟投和赎回/止盈必须从既有定投计划中选择，避免产品名手输错误生成重复持仓。投顾流水只记录确认成功的金额，不要求录入份额或单位净值。
