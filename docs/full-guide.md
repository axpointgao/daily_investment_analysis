...
- 投顾是独立账户类型，不归入银行账户。Web 端按产品记录平台、产品名称、可选产品代码/外部 ID，并在产品类型中选择“投顾组合”或“定投计划”。投顾组合的“买入”手动填写产品信息，追加和赎回必须从既有投顾组合中选择；定投计划的首次买入手动填写产品信息，跟投和赎回/止盈必须从既有定投计划中选择，避免产品名手输错误生成重复持仓。投顾流水只记录确认成功的金额，不要求录入份额或单位净值。
- 投顾产品当前不自动读取且慢、支付宝、陆基金/陆金所等账户数据。估值按金额口径自动顺推：未维护价值时使用累计投入减累计赎回，维护价值后使用最近一次“价值更新”加后续投入减后续赎回。用户可在 Web 端“价值更新”入口选择投顾产品并录入当前总价值。底层基金穿透本轮不提供入口；如用户未来选择逐只录入底层基金，应避免同时把投顾产品总价值重复计入。配置 `YINGMI_API_KEY` 后，持仓“深度诊断”和诊基 Agent 会优先使用盈米 StarGate 的投顾策略、组合诊断和基金专业分析能力，手工账本仍作为资产金额与持仓事实来源。
