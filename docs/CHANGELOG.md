...
- [新功能] Web 策略选股新增可持久化策略库，支持保存策略名称、说明、完整选股语句、回测状态和最近一次运行结果，并通过弹窗更换当前策略。
- [改进] Web 策略选股隐藏本地评分维度多选，改为默认质量优先二次评分，让主界面聚焦当前策略问句和运行结果。
- [新功能] Web 新增策略选股页面和选股 API，支持本地策略初筛与问财客户端 Excel 候选导入，并结合本地行情、技术指标和基本面做候选展示。
- [改进] 回测页在没有可回测历史分析记录时显示冷却期与强制重跑提示。
...
- [修复] TiantianFundApi 调用新增短重试，降低云端代理偶发 500 对基金数据源测试、基金分析和持仓基金估值的影响。
...
- [修复] 修复持仓服务 `_data_manager` 初始化位置错误导致股票名称解析或实时价刷新可能失败的问题。
- [新功能] 银行理财接入问财 Skills，可按产品名称查询银行理财产品并按确认日单位净值记录份额，刷新持仓时可用最新净值估值；问财失败时保留手动价值更新。
- [新功能] 投顾流水接入盈米 StarGate 产品查询；投顾组合按用户选择的流水日期查询历史净值并确认份额，后续刷新可用最新净值估值；定投计划只记录查询到的产品信息，不进入净值自动估值。
- [改进] 投顾产品查询失败或历史净值缺失时不阻断手工录入，未匹配净值的投顾组合继续按金额口径并支持手动价值更新。
- [改进] Web 持仓页录入流水表单增加字段标题、下拉箭头和短按钮宽度限制，并收敛表单最大宽度，提升银行理财、投顾、保险等多字段录入的可读性。
- [修复] Web 持仓明细对净值型银行理财展示份额、成本净值和单位净值，并将问财净值来源显示为中文。
- [改进] 持仓资产分析增强保险账户结构化上下文，单保险账户分析升级为基于现金价值、缴费压力和领取流水的保单资产体检。
- [改进] Web 持仓页支持在单账户视图编辑账户名称和机构/平台，币种与账户类型保持只读。
- [新功能] Web 持仓页支持在同类型账户之间按单产品/单标的转移资产，并在转移前预览将迁移的源数据。
...
- [改进] Web 持仓页数字货币数量录入和展示支持小数点后 8 位。
- [改进] Web 持仓页主加载不再调用风险接口，并将说明文案改为资产分析、持仓盈亏颜色改为盈红亏绿。
- [改进] 持仓快照默认优先使用缓存/最近快照，Web “刷新数据”才触发在线实时价刷新，并避免基金名称解析走股票数据源。
- [改进] 问股 Agent 对持仓/组合配置/ETF 替换类问题默认跳过新闻搜索，ETF/指数基本信息走轻量返回，并优化长耗时流式问答的前端历史兜底。
- [改进] 银行账户拆分活期、定期存款和银行理财三类录入，银行理财按金额流水和当前总价值维护，登记编码、投资性质、风险等级和期限信息均为选填。
- [改进] Web 银行资产录入支持从已有定期/理财持仓选择取出或赎回，定期按单笔流水区分同名产品，银行理财追加、赎回和价值更新均选择既有产品。
- [新功能] 持仓管理新增独立投顾账户，支持投顾组合与定投计划按金额记录投入、赎回和当前价值，并纳入全账户资产快照与资产分布。
- [改进] 投顾账户改为金额口径，支持投顾组合与定投计划两类产品流水，价值更新录入当前总价值，不再要求投顾产品份额和单位净值。
- [改进] 银行理财改为金额口径，买入和追加只记录支付金额，价值更新录入当前总价值，不再要求份额和单位净值。
- [改进] Web 投顾流水录入区分新产品买入和后续投入，追加买入、跟投和赎回均按产品类型选择既有产品，避免手输产品名生成重复持仓。
- [新功能] 持仓管理新增保险账户，支持保单、缴费、返还/领取、现金价值更新和分期缴费进度，并纳入全账户资产快照与资产分布。
- [改进] 保险流水录入按保单险种、设计类型和状态动态收敛可操作项，并在服务层阻止明显不适配的保险事件。
- [新功能] 基金投顾链路接入盈米 StarGate，首页基金分析、诊基 Agent 和持仓深度诊断优先使用盈米专业诊断，天天基金与自建 TiantianFundApi 保留为基础数据补充和失败兜底。
- [改进] 设置页 Agent 配置新增盈米 StarGate 连通性测试和额度配置，并将股票策略、天天基金 Skills、盈米投顾能力的说明拆清楚，降低配置黑盒感。
- [改进] Agent 设置页按基金投顾、股票问股和后台自动化分组展示配置，并默认收起高成本高级项，减少用户理解配置影响的成本。
- [改进] Agent 设置页隐藏固定的盈米服务地址，基金投顾配置改为 API Key、启用开关、分析深度和数据策略，失败降级保持默认体验。
- [改进] 设置页新增持仓分析配置，可查看默认 Prompt 并按账户场景覆盖资产分析、深度诊断和财富报告的写作要求。
- [改进] 首页基金分析提交后立即显示任务进度，并在基金 SSE 断开或延迟时通过状态接口轮询兜底；基金基础补充数据改为并发获取，减少收益、排名、经理和评级阶段等待。
- [改进] 首页基金诊断默认复用当天同基金同报告类型的历史结果，并记录阶段耗时与压缩 LLM 输入载荷，减少重复外部调用和诊断等待。
- [修复] 修复系统设置接口未接受持仓分析配置分类导致设置页加载失败的问题。
- [修复] 修复场外基金完整诊断邮件中评级数据直接显示为代码字典的问题，改为 Markdown 表格展示。
- [改进] 诊基 Agent 压缩基金工具结果并复用同轮重复工具调用，降低上下文 token 消耗和重复外部查询。
- [改进] 持仓资产分析记录独立 LLM 用量类型，并输出账号范围、模式、盈米状态和耗时日志，便于测试排查。
- [修复] 持仓深度诊断在基金估值缺失时使用持仓成本作为盈米组合权重兜底，避免 0 市值缓存静默跳过专业诊断。
- [改进] 持仓深度诊断强化专业数据覆盖、口径一致性、风险优先级、基金/投顾专项和观察清单章节，提升相对普通分析的增量信息密度。
- [改进] 持仓资产分析收口为单一资产分析报告，盈米仅作为基金/投顾专项输入参与最终整合，并统一跨币种金额口径与 ETF 行业集中度处理。
