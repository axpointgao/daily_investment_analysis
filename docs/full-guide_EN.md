...
- Bank accounts support demand/cash, term deposits, and bank wealth products. In the bank ledger form, the first date is the ledger event date; the product term dates are the interest start date and maturity date, which are required for term deposits and optional for bank wealth products. Bank wealth purchases record the paid amount, confirmed shares, and confirmed NAV; that confirmed NAV is saved as the product's manual NAV for the purchase date. Later NAV updates are maintained from the Web "NAV update" entry, and valuation uses held shares multiplied by the latest NAV.
- Advisory accounts track products by platform, product name, optional product code/external ID, and a product type of advisory combo or DCA plan. The Web entry flow separates "new combo buy" from "additional buy": new products use manual product fields, while additional buys and redemptions must select an existing advisory combo. DCA plans use manual product fields for the initial buy, while DCA contributions, manual follow-buys, and redemptions/take-profit entries must select an existing DCA plan, which avoids duplicate holdings caused by product-name typos. Advisory ledgers are amount-based and no longer require product shares or unit NAV. Valuation uses cumulative invested minus redeemed amounts until the user records a Web "value update"; after that, the latest value is rolled forward by later investments and redemptions. Underlying fund look-through is not exposed in this release and should not be double-counted with the advisory product value.
