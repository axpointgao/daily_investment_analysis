...
- Insurance is an independent portfolio account type for savings/wealth-style policies such as annuity, whole-life, endowment, universal, and unit-linked policies. The Web page lets users create a policy, then record premiums, cash-value/account-value updates, annuity/survival/maturity benefits, dividends, partial withdrawals, refunds, and surrender proceeds. Only the policy name is required; payment schedule fields remain optional and are used to show paid periods and the next premium date when available.
- Insurance valuation prefers the user-maintained cash value/account value. Before a value update exists, the snapshot estimates value as paid premiums minus cumulative returns and marks the position as estimated. Insurance returns are tracked as policy cash flows and are not automatically mirrored into bank ledgers, avoiding duplicate bank-account entries.
- Portfolio asset analysis includes a structured insurance asset summary. In all-account analysis, insurance is treated as a long-duration, low-liquidity cash-value asset. When analyzing a single insurance account, the report uses recorded premiums, cash value, received benefits, payment progress, next premium date, and value-update freshness for a policy-asset checkup. It does not read policy contracts, judge medical/critical-illness/accident coverage adequacy, handle claims, or provide insurance purchase recommendations.
- Bank accounts support demand/cash, term deposits, and bank wealth products. In the bank ledger form, the first date is the ledger event date; the product term dates are the interest start date and maturity date, which are required for term deposits and optional for bank wealth products. Bank wealth products are amount-based: the first purchase records the bank, product name, paid amount, and purchase date, while registration code, investment nature, risk level, income mode, and term dates are optional. Additional purchases and redemptions must select an existing wealth product. Later value updates are maintained from the Web "value update" entry by recording the current total value; before a value update exists, valuation is estimated from purchase, additional purchase, and redemption amounts.
- Advisory accounts track products by platform, product name, optional product code/external ID, and a product type of advisory combo or DCA plan. When `YINGMI_API_KEY` is configured, new product entries can search Yingmi StarGate by product name. After the user picks a candidate, advisory combos fetch the historical NAV for the selected ledger date and calculate confirmed units; DCA plans only store the matched product metadata for asset analysis and never enter NAV-based auto valuation. Search failures, missing configuration, or missing historical NAV do not block manual amount-based entry.
- Additional advisory-combo buys and redemptions must select an existing advisory combo. DCA follow-buys and redemption/take-profit entries must select an existing DCA plan, avoiding duplicate holdings caused by product-name typos. Advisory combos matched with historical NAV can use Yingmi latest NAV during portfolio refresh; unmatched advisory combos and DCA plans keep amount-based valuation and can still be corrected with the Web "value update" flow. Underlying fund look-through is not exposed in this release and should not be double-counted with the advisory product value.
