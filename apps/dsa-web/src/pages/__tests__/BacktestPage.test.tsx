...
describe('BacktestPage', () => {
  it('renders shared surface inputs and prediction tracking outputs', async () => {
    ...
    const filterInput = await screen.findByPlaceholderText('按股票代码筛选（留空查看全部）');
    const windowInput = screen.getByPlaceholderText('10');

    expect(filterInput).toHaveClass('border-input');
    expect(filterInput).toHaveClass('focus-visible:ring-ring/50');
    expect(windowInput).toHaveClass('border-input');
    expect(windowInput).toHaveClass('focus-visible:ring-ring/50');
  ...
  it('filters results with stock code, window, and analysis date range when clicking 筛选', async () => {
    ...
    const filterInput = await screen.findByPlaceholderText('按股票代码筛选（留空查看全部）');
    const windowInput = screen.getByPlaceholderText('10');
    const fromInput = screen.getByLabelText('分析日期开始');
    const toInput = screen.getByLabelText('分析日期结束');
  ...
  it('switches to next-day validation with the 1D shortcut', async () => {
    ...
    expect(screen.getByText('次日验证模式会将智能预测与下一个交易日收盘表现进行对比。')).toBeInTheDocument();
  });
});
